...
class AppTest {
    ...
    void testDirectory() throws Exception {
        CompletableFuture<Long> dirSize = App.getDirectorySize("src/test/resources/dir");
        Long result = dirSize.get();
        assertThat(result).isEqualTo(26L);
    }

    @Test
    void testEmptyDirectory() throws Exception {
        CompletableFuture<Long> dirSize = App.getDirectorySize("src/test/resources/empty_dir");
        Long result = dirSize.get();
